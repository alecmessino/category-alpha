 ...
 *
 * THE ONE COMPOSITION THE ARCHIVE AUTHORISES is P(forms) x P(reaches X | forms), for INTENSITY
 * only. The archive says so itself, and says in the same breath that landfall does NOT decompose
 * that way. So intensity is composed here and landfall never is.
 *
...
function parseOutlookAreas(twoText) {
  const out = [];
...
/** ATCF b-deck BEST rows — used only for the genesis fix and the latest analysed position. */
function bestRows(dat) {
  const rows = [];
...
const LEVELS = ["td", "ts", "cat1", "cat2", "cat3", "cat4", "cat5"];

function pack(a, label) {
  const lv = (k) => {
...
/* P(forms) x P(reaches X | forms). INTENSITY ONLY — the archive is explicit that landfall does
   not decompose this way, and nothing here composes one. */
function compose(formationPct, packed) {
  if (!packed.sufficient)
    return { available: false, formationPct,
             reason: "CONDITIONAL REFUSED — the analog pool is below the sample gate, so there is "
                   + "no P(reaches X | forms) to multiply. No composed number exists." };
  const levels = {};
  for (const k of ["ts", "cat1", "cat2", "cat3"]) {
    const o = packed.levels[k];
    levels[k] = { composed: (formationPct / 100) * o.rate,
                  ci: [(formationPct / 100) * o.ci[0], (formationPct / 100) * o.ci[1]] };
  }
  return { available: true, formationPct, levels,
           note: "P(forms) × P(reaches X | forms), for intensity only. The formation term is "
               + "NHC's published 7-day chance; the conditional term is the archive's. Landfall "
               + "does not decompose this way and is never composed here." };
}
...
async function main() {
  ...
  const A = {
    ...
    fourteenE: pack(analogs(genesisFix.lat, genesisFix.lonE, { minPoolSeason: 1971 }),
                    "Fourteen-E — conditioned on its genesis fix, 1971+"),
  };

  const genesisMs = Date.parse(genesisFix.atZ);
  ...
  const payload = {
    ...
    storms: {
      fourteenE: {
        ...
        genesisFix, latestBest,
        forecastIssuedZ: tcm.issuedZ, forecast: tcm.rows,
        /* NHC's own forecast laid against the analog distribution. Both measured as hours from a
           first fix — a comparison, never a claim that either is wrong. */
        forecastVsAnalogs: firstCat1 ? {
          firstCat1,
          hoursFromGenesis: (Date.parse(firstCat1.validZ) - genesisMs) / 3600e3,
          analogHoursToCat1: A.fourteenE.timeToEvent.cat1 || null,
          caveat: "The operational b-deck's first fix and the archive's IBTrACS genesis record are "
                + "different conventions and can differ by hours.",
        } : null,
      },
    ...
    atlas: A,
    composed: { d1: compose(g1.pct7d, A.d1), d2: compose(g2.pct7d, A.d2) },
    limits: [
      ...
      "A conditional rate assumes a tropical cyclone forms. Composed with a formation chance it becomes unconditional; that composition is authorised for intensity and never for landfall.",
  ...
  if (payload.composed.d2.available)
    console.log(`  D2 composed     Cat 1 ${payload.composed.d2.levels.cat1.composed.toFixed(1)}%`);
