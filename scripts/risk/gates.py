...
from tec import ROOT, RAW, COASTLINES, declared_inputs, sha256, load_all, reconcile_lead_labels

D = ROOT / "docs" / "risk" / "lowell-2026"
BUILD = Path(__file__).parent / ".build"
M = json.loads((D / "lowell-2026.manifest.json").read_text())
PDF = D / "Millibar-Lowell-2026-Event-Brief.pdf"
BRIEF = BUILD / "brief.html"
...
def latest_source_stamp() -> datetime:
    ...
    stamps = [datetime.strptime(m.group(1), "%Y%m%d%H%M").replace(tzinfo=timezone.utc)
              for m in (re.search(r"\.(\d{12})\.", f.name) for f in declared_inputs()) if m]
    if not stamps:
        raise SystemExit("no timestamped source products: cannot date the PDF deterministically")
    return max(stamps)
...
for label, r in (("first-reported 46A", op["residual_vs_first_reported_fix"]),
                 ("revised advisory 47", op["residual_vs_revised_fix"])):
...
gate("PDF section D states the vintage sentence",
     tight("Same forecast. Same valid time. The verifying observation changed with the source vintage.") in pdf_tight)
...
missing = [f.name for f in declared_inputs() if not f.is_file()]
gate("every declared build input exists in the checkout", not missing, ", ".join(missing[:4]))
unhashed = [f.name for f in declared_inputs() if f.name not in reg]
gate("every declared build input is in the source register", not unhashed, ", ".join(unhashed[:4]))
bad = [f.name for f in declared_inputs() if f.name in reg and reg[f.name]["sha256"] != sha256(f)]
...
gate("no source is registered that the build does not read",
     not [k for k in reg if k not in {f.name for f in declared_inputs()}])
