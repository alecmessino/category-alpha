...
function gratLayer(P, opts) {
  ...
  return `<g class="grat">`
    ...
    + g.labels.map((l) => `<text x="${l.x}" y="${l.y}" text-anchor="${l.anchor}" `
      + `font-family="IBM Plex Mono,monospace" font-size="6.4" fill="#94a3b8">${esc(l.text)}</text>`).join("")
    + `</g>`;
}
...
function mark(P, lat, lon, { color, label, sub, kind = "cell", anchor = "start", dy = 0, dx = 0, leader = false }) {
  const [x, y] = P(lon, lat);
  const r = 4.2;
  ...
  return glyph + lead
    ...
    + `font-family="IBM Plex Mono,monospace" font-size="7.2" font-weight="600" fill="${color}" `
    + `letter-spacing=".4">${esc(label)}</text>`
    + (sub ? `<text x="${f2(tx)}" y="${f2(y + 7.4 + dy)}" text-anchor="${anchor}" `
      + `font-family="IBM Plex Mono,monospace" font-size="6.2" fill="#475569">${esc(sub)}</text>` : "");
}
...
export function basinPlate(D, { width = 700, height = 300 } = {}) {
  ...
  const outlookPaths = [al97, ep95].filter(Boolean).map((o) =>
...
export const LEGEND = {
  ...
  genesisCell: { kind: "sq", color: INK.genesis, fill: "rgba(15,23,42,.08)", label: "observed genesis + query radius" },
