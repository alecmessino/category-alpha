...
export const LEGIBILITY_CUTS = {
  ...
  "B-97L-gulf-event-dossier.html": [
    ...
    { kind: "compressed", block: "comparison strip → a WHAT ATLAS ADDS line", px: 129,
      ...
      why: "the frequency panel took the row anatomy's height and the sample boundary moved to sheet 2 to make room; D carries the strip in full" },
  ...
  "B1-97L-reinsurance-ils-parametric.html": [
    ...
    { kind: "compressed", block: "comparison strip → a WHAT ATLAS ADDS line", px: 115,
      instead: "one sentence above the cite panel",
      why: "the frequency panel took the row anatomy's height and the page stays one sheet; D carries the strip in full" },
    { kind: "compressed", block: "two headed cite blocks (published, 150 km refusal) → one CITE THIS COHORT panel", px: 0,
      instead: "the lead cite string once; REPLAY PUBLISHED COHORT, REPLAY 150 KM REFUSAL and REPLAY ALL MONTHS as labelled links to the exact URLs, each query string printed beneath, tagged, at the legal size",
      why: "the visual-translation pass: a label a reader can act on in place of a 106-character query string as link text; the same three cohorts B's cite panel reopens" },
  ],
  "B2-97L-energy-weather-trading.html": [
    { kind: "compressed", block: "comparison strip → a WHAT ATLAS ADDS line", px: 129,
      instead: "one sentence above the cite block",
      why: "the visual-translation pass put the seasonal-timing quantiles at the top of the page; D carries the strip in full" },
    { kind: "cut", block: "answers rail (three columns)", px: 107,
      instead: "the live line under the masthead carries what is happening now; WHAT ATLAS ADDS carries the rest",
      why: "the member timeline is the finding and belongs at the top of the page; the rail restated the masthead" },
    { kind: "compressed", block: "analog-track plate + seasonal-timing table → the member timeline", px: 0,
      instead: "one figure: twelve members on one hours-from-genesis axis, each threshold reached and each recorded crossing as a held mark, the published quantiles beneath on the same axis",
      why: "a desk's question is when and what happened, member by member; the plate answered only where, and the quantile table hid that n=22 CONUS crossings are twelve members. Drawn geometry stays on B" },
    { kind: "compressed", block: "COMMERCIAL READING — what the rows cover (exposure-map) → rows", px: 0,
      instead: "ANITA 1977 and ALICIA 1983 are rows of the timeline with their crossings; MEXICO — ≥64 kt is a row of the scored table",
      why: "the paragraph restated two timeline rows and one contract row" },
    { kind: "compressed", block: "the reading of the frequency bands, inside the rows column → full width beneath the band", px: 14,
      instead: "the same sentence, two lines instead of three",
      why: "the rows column set the height of the plate band" },
    { kind: "cut", block: "a drawn-interval column in the five-row scored table", px: 0,
      instead: "the printed 95% Wilson column the table already carries",
      why: "tried in the visual-translation pass: it cost the analog plate a fifth of its width, and at that width the plate could not hold its labels at the type floor. Five printed intervals did not need the picture; B1 and C carry the drawn interval on their nineteen-row ladders" },
  ],
  ...
  "E-discrete-event-contract-evidence.html": [
    { kind: "compressed", block: "evidence-bridge table (five rows) → the joint matrix and a three-row bridge", px: 0,
      instead: "the contract's own 2 x 2 — reached Cat 4 against ≥64 kt at a CONUS crossing — with both published margins and an interior the archive holds no cell of; then needs → holds → verdict for the definition, the sub-CONUS variants and the named history",
      why: "the first row of the table said the joint row does not exist; the matrix shows it, with the margins that must not be multiplied printed as margins" },
    { kind: "cut", block: "masthead lede", px: 15,
      instead: "the title, which says the same thing in fewer words",
      why: "the lede restated the title; the page needed the line for the joint matrix" },
    { kind: "compressed", block: "LIVE STATUS box → one line beside the citation", px: 60,
      instead: "the same operational sentence and the genesis lock, at the foot of the page",
      why: "live status is context, not evidence; the finding takes the top of the page" },
