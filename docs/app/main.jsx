          ...
          <div style={{ display: "grid", gridTemplateColumns: narrow ? "1fr" : "minmax(0,1fr) 320px" }}>
            ...
            <aside style={{ background: "var(--surface-card)", borderLeft: "1px solid var(--border-dim)", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: "var(--border-dim)", borderBottom: "1px solid var(--border-dim)" }}>
                ...
                <STt variant="metric" label="EDGE" value={snap.edgePct == null ? "—" : (snap.edgePct >= 0 ? "+" : "") + snap.edgePct.toFixed(1)} unit={snap.edgePct == null ? "" : "%"} sub={snap.edgePct == null ? "no model" : "model − mkt"} />
              ...
              <div style={{ padding: "12px 15px", flex: 1, overflowY: "auto" }}>
                ...
                {snap.edgePct != null && <Anno tone={snap.edgePct >= 0 ? "pos" : "neu"} icon="◧" title={"Edge " + (snap.edgePct >= 0 ? "+" : "") + snap.edgePct.toFixed(1) + "%"} desc={"Model anchor vs market on Cat 4+."} />}
                <Anno tone="neu" icon="➤" title="Tracking" desc={snap.wind + " kt, moving " + S.movement + "."} />
                <div style={{ marginTop: 10, fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--text-2)", opacity: .8 }}>Interpretation, not observation. Research-only — no execution, no advice. Live NHC + market data.</div>
              </div>
            </aside>
          </div>
