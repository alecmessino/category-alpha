...
function buildContext(stormId, frame) {
  ...
  const contracts = MT.contracts.map((c) => { const k = MTX.kellyFor(c, frame, 10000, 0.25); return c.label + ": edge " + k.edge.toFixed(1) + "%, mkt " + Math.round(k.market) + "%" + (k.noBet ? " (no bet)" : ", Q-Kelly $" + k.allocation + (k.liqCapped ? " LIQ-CAPPED" : "")); });
  ...
  return {
    ...
    pressure_mb: s.pressure, wind_kt: s.wind, model_cat4: Math.round(s.model * 100) + "%",
    market_cat4: Math.round(s.market * 100) + "%", edge_pct: s.edgePct.toFixed(1),
...
function MT_Console({ stormId, frame }) {
  const [log, setLog] = React.useState([{ role: "sys", text: "Category Alpha assistant ready. Type `help`, or ask about the current evidence, confidence, or edge." }]);
  ...
  async function run(cmd) {
    ...
    if (lc === "status") {
      ...
      setLog((l) => [...l, { role: "sys", text: `as-of ${ctx.as_of} · ${ctx.storm}\n${ctx.pressure_mb} mb · ${ctx.wind_kt} kt · model ${ctx.model_cat4} vs mkt ${ctx.market_cat4} · edge ${ctx.edge_pct}% · confidence tier ${ctx.confidence_tier}` }]); return;
    }
