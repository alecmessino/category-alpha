...
function MT_Probability({ stormId, frame }) {
  const s = MTX.snap(stormId, frame);
  return (
    <P title="Probability — Cat4+" right={<BG tone="special">ANCHOR ONLY</BG>}
      footer={<PF source="NHC anchor + model consensus" latency="40m" version="12z" tier="B" />}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 10 }}>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 32, fontWeight: 800, color: "var(--accent)" }}>{Math.round(s.model * 100)}%</span>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-2)" }}>Category Alpha anchor · vs mkt {Math.round(s.market * 100)}%</span>
      </div>
      {MT.models.map((m) => (
      ...
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, color: "var(--text-2)", marginTop: 8 }}>Probability engine deferred — anchor is NHC-intensity-derived, not a fitted model.</div>
    </P>
  );
}
...
function MT_EdgeMatrix({ frame, bankroll, stake, setBankroll, setStake, selection, onSelect, dense }) {
  ...
  return (
    <P pad={false} title="Edge Matrix — Q-Kelly Allocation" right={<BG tone="live" dot>LIVE</BG>}
      footer={<PF source="Category Alpha × Kalshi (seeded)" latency="5m" version="1.2.4" tier="B" />}>
...
function MT_Markets({ frame, selection, onSelect, dense }) {
  ...
  const rows = MT.contracts.map((c) => {
    ...
    return { c, px, model, edge: (model - px) * 100, d: (px - prev) * 100, hist: MTX.priceHist(c, frame, 12) };
  });
  ...
  return (
    <P pad={false} title="Prediction Markets — Kalshi board" right={<BG tone="live" dot>{rows.length} MKTS</BG>}
      footer={<PF source="Kalshi order-driven (seeded)" latency="5m" version="mkt-77" tier="C" />}>
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: dense ? 11 : 12 }}>
          ...
          <tbody>{rows.map(({ c, px, model, edge, d, hist }) => {
            ...
            const eStyle = edge >= 15 ? { color: "var(--edge-glow)", textShadow: "var(--glow-edge)" } : edge > 0 ? { color: "var(--pos)" } : { color: "var(--text-2)" };
            return (
              <tr key={c.id} onClick={() => onSelect(c.id)} style={{ cursor: "pointer", background: on ? "color-mix(in srgb,var(--accent) 12%,transparent)" : "transparent" }}>
                ...
                <td style={{ ...cell, color: "var(--text-1)", fontWeight: 700 }}>{Math.round(px * 100)}¢</td>
                <td style={{ ...cell, color: d >= 0 ? "var(--pos)" : "var(--neg)" }}>{d >= 0 ? "+" : ""}{d.toFixed(1)}</td>
                <td style={{ ...cell, color: "var(--text-2)" }}>{Math.round(model * 100)}%</td>
                <td style={{ ...cell, fontWeight: 800, ...eStyle }}>{edge >= 0 ? "+" : ""}{edge.toFixed(1)}</td>
...
function MT_OrderBook({ contractId, frame, dense }) {
  ...
  let cum = 0;
  return (
    <P pad={false} title="Order Book & Liquidity" right={<BG tone="warn">CAP ${Math.round(ob.liquidityCap / 1000)}k</BG>}
      footer={<PF source="Kalshi depth (seeded)" latency="5m" version="ob-77" tier="C" />}>
